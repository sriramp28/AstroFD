#!/usr/bin/env python3
# core/grhd_core.py
# GRHD (fixed metric) with simple Schwarzschild source terms (approximate).
import numpy as np
from core import srhd_core
from core import eos
from core import gr_metric

GR_METRIC = "minkowski"
GR_MASS = 1.0
ORTHONORMAL_FLUX = True
GR_SPIN = 0.0

def configure(params):
    global GR_METRIC, GR_MASS, ORTHONORMAL_FLUX, GR_SPIN
    GR_METRIC = str(params.get("GR_METRIC", "minkowski")).lower()
    GR_MASS = float(params.get("GR_MASS", GR_MASS))
    GR_SPIN = float(params.get("GR_SPIN", GR_SPIN))
    ORTHONORMAL_FLUX = bool(params.get("ORTHONORMAL_FLUX", ORTHONORMAL_FLUX))

def compute_rhs_grhd(pr, dx, dy, dz, offs_x, ng):
    nx, ny, nz = pr.shape[1], pr.shape[2], pr.shape[3]
    pr_use = pr
    if ORTHONORMAL_FLUX and GR_METRIC in ("schwarzschild", "kerr-schild"):
        pr_use = pr.copy()
        for i in range(ng, nx-ng):
            x = (offs_x + (i - ng) + 0.5) * dx
            for j in range(ng, ny-ng):
                y = (j - ng + 0.5) * dy
                for k in range(ng, nz-ng):
                    z = (k - ng + 0.5) * dz
                    if GR_METRIC == "schwarzschild":
                        alpha, _bx, _by, _bz, _dax, _day, _daz = gr_metric.schwarzschild_ks_lapse_shift_and_grad(x, y, z, GR_MASS)
                    else:
                        alpha, _bx, _by, _bz, _dax, _day, _daz = gr_metric.kerr_schild_lapse_shift_and_grad(x, y, z, GR_MASS, GR_SPIN)
                    ax = max(alpha, 1e-12)
                    pr_use[1, i, j, k] = pr[1, i, j, k] / ax
                    pr_use[2, i, j, k] = pr[2, i, j, k] / ax
                    pr_use[3, i, j, k] = pr[3, i, j, k] / ax

    if srhd_core.RECON_ID == 1:
        rhs = srhd_core.compute_rhs_ppm(pr_use, nx, ny, nz, dx, dy, dz)
    elif srhd_core.RECON_ID == 2:
        rhs = srhd_core.compute_rhs_weno(pr_use, nx, ny, nz, dx, dy, dz)
    else:
        rhs = srhd_core.compute_rhs_muscl(pr_use, nx, ny, nz, dx, dy, dz)

    if GR_METRIC == "minkowski":
        return rhs

    for i in range(ng, nx-ng):
        x = (offs_x + (i - ng) + 0.5) * dx
        for j in range(ng, ny-ng):
            y = (j - ng + 0.5) * dy
            for k in range(ng, nz-ng):
                z = (k - ng + 0.5) * dz
                if GR_METRIC == "schwarzschild":
                    _alpha, dlnadx, dlnady, dlnadz = gr_metric.schwarzschild_iso_lapse_and_grad(x, y, z, GR_MASS)
                else:
                    _alpha, _bx, _by, _bz, dlnadx, dlnady, dlnadz = gr_metric.kerr_schild_lapse_shift_and_grad(x, y, z, GR_MASS, GR_SPIN)

                rho, vx, vy, vz, p = pr[0,i,j,k], pr[1,i,j,k], pr[2,i,j,k], pr[3,i,j,k], pr[4,i,j,k]
                v2 = vx*vx + vy*vy + vz*vz
                if v2 >= 1.0:
                    v2 = 1.0 - 1e-14
                W = 1.0 / np.sqrt(1.0 - v2)
                h = eos.enthalpy(rho, p)
                w = rho * h * W * W + p
                _, Sx, Sy, Sz, _ = srhd_core.prim_to_cons(rho, vx, vy, vz, p)

                rhs[1,i,j,k] += w * dlnadx
                rhs[2,i,j,k] += w * dlnady
                rhs[3,i,j,k] += w * dlnadz
                rhs[4,i,j,k] += Sx*dlnadx + Sy*dlnady + Sz*dlnadz

    return rhs

def step_ssprk2(pr, dx, dy, dz, dt, offs_x, ng):
    nx, ny, nz = pr.shape[1], pr.shape[2], pr.shape[3]
    nt = srhd_core.N_PASSIVE
    U0 = np.zeros((5, nx, ny, nz))
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                U0[:,i,j,k] = srhd_core.prim_to_cons(pr[0,i,j,k], pr[1,i,j,k], pr[2,i,j,k], pr[3,i,j,k], pr[4,i,j,k])

    rhs1 = compute_rhs_grhd(pr, dx, dy, dz, offs_x, ng)
    U1   = U0 + dt*rhs1[0:5]

    pr1 = np.zeros_like(pr)
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                pr1[0:5,i,j,k] = srhd_core.cons_to_prim(U1[0,i,j,k], U1[1,i,j,k], U1[2,i,j,k], U1[3,i,j,k], U1[4,i,j,k])
    if pr.shape[0] > 5 and srhd_core.PASSIVE_OFFSET > 5:
        pr1[5:srhd_core.PASSIVE_OFFSET] = pr[5:srhd_core.PASSIVE_OFFSET]
    if nt > 0:
        pr1[srhd_core.PASSIVE_OFFSET:srhd_core.PASSIVE_OFFSET+nt] = pr[srhd_core.PASSIVE_OFFSET:srhd_core.PASSIVE_OFFSET+nt] + dt*rhs1[5:5+nt]

    rhs2 = compute_rhs_grhd(pr1, dx, dy, dz, offs_x, ng)
    U2   = 0.5*(U0 + U1 + dt*rhs2[0:5])

    out  = np.zeros_like(pr)
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                out[0:5,i,j,k] = srhd_core.cons_to_prim(U2[0,i,j,k], U2[1,i,j,k], U2[2,i,j,k], U2[3,i,j,k], U2[4,i,j,k])
    if pr.shape[0] > 5 and srhd_core.PASSIVE_OFFSET > 5:
        out[5:srhd_core.PASSIVE_OFFSET] = pr1[5:srhd_core.PASSIVE_OFFSET]
    if nt > 0:
        t0 = pr[srhd_core.PASSIVE_OFFSET:srhd_core.PASSIVE_OFFSET+nt]
        t1 = pr1[srhd_core.PASSIVE_OFFSET:srhd_core.PASSIVE_OFFSET+nt]
        out[srhd_core.PASSIVE_OFFSET:srhd_core.PASSIVE_OFFSET+nt] = 0.5*(t0 + t1 + dt*rhs2[5:5+nt])
    return out

def step_ssprk3(pr, dx, dy, dz, dt, offs_x, ng):
    nx, ny, nz = pr.shape[1], pr.shape[2], pr.shape[3]
    nt = srhd_core.N_PASSIVE
    U0 = np.zeros((5, nx, ny, nz))
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                U0[:,i,j,k] = srhd_core.prim_to_cons(pr[0,i,j,k], pr[1,i,j,k], pr[2,i,j,k], pr[3,i,j,k], pr[4,i,j,k])

    rhs1 = compute_rhs_grhd(pr, dx, dy, dz, offs_x, ng)
    U1 = U0 + dt*rhs1[0:5]

    pr1 = np.zeros_like(pr)
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                pr1[0:5,i,j,k] = srhd_core.cons_to_prim(U1[0,i,j,k], U1[1,i,j,k], U1[2,i,j,k], U1[3,i,j,k], U1[4,i,j,k])
    if pr.shape[0] > 5 and srhd_core.PASSIVE_OFFSET > 5:
        pr1[5:srhd_core.PASSIVE_OFFSET] = pr[5:srhd_core.PASSIVE_OFFSET]
    if nt > 0:
        pr1[srhd_core.PASSIVE_OFFSET:srhd_core.PASSIVE_OFFSET+nt] = pr[srhd_core.PASSIVE_OFFSET:srhd_core.PASSIVE_OFFSET+nt] + dt*rhs1[5:5+nt]

    rhs2 = compute_rhs_grhd(pr1, dx, dy, dz, offs_x, ng)
    U2 = 0.75*U0 + 0.25*(U1 + dt*rhs2[0:5])

    pr2 = np.zeros_like(pr)
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                pr2[0:5,i,j,k] = srhd_core.cons_to_prim(U2[0,i,j,k], U2[1,i,j,k], U2[2,i,j,k], U2[3,i,j,k], U2[4,i,j,k])
    if pr.shape[0] > 5 and srhd_core.PASSIVE_OFFSET > 5:
        pr2[5:srhd_core.PASSIVE_OFFSET] = pr1[5:srhd_core.PASSIVE_OFFSET]
    if nt > 0:
        t0 = pr[srhd_core.PASSIVE_OFFSET:srhd_core.PASSIVE_OFFSET+nt]
        t1 = pr1[srhd_core.PASSIVE_OFFSET:srhd_core.PASSIVE_OFFSET+nt]
        pr2[srhd_core.PASSIVE_OFFSET:srhd_core.PASSIVE_OFFSET+nt] = 0.75*t0 + 0.25*(t1 + dt*rhs2[5:5+nt])

    rhs3 = compute_rhs_grhd(pr2, dx, dy, dz, offs_x, ng)
    U3 = (1.0/3.0)*U0 + (2.0/3.0)*(U2 + dt*rhs3[0:5])

    out = np.zeros_like(pr)
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                out[0:5,i,j,k] = srhd_core.cons_to_prim(U3[0,i,j,k], U3[1,i,j,k], U3[2,i,j,k], U3[3,i,j,k], U3[4,i,j,k])
    if pr.shape[0] > 5 and srhd_core.PASSIVE_OFFSET > 5:
        out[5:srhd_core.PASSIVE_OFFSET] = pr2[5:srhd_core.PASSIVE_OFFSET]
    if nt > 0:
        t0 = pr[srhd_core.PASSIVE_OFFSET:srhd_core.PASSIVE_OFFSET+nt]
        t2 = pr2[srhd_core.PASSIVE_OFFSET:srhd_core.PASSIVE_OFFSET+nt]
        out[srhd_core.PASSIVE_OFFSET:srhd_core.PASSIVE_OFFSET+nt] = (1.0/3.0)*t0 + (2.0/3.0)*(t2 + dt*rhs3[5:5+nt])
    return out
